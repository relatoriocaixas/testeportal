...
// ===== Helpers =====
function formatDateBR(ts) { return (ts?.toDate ? ts.toDate() : ts) instanceof Date ? ts.toDate?.() || ts : ts; }
function parseDateInput(value){ const [y,m,d]=value.split('-').map(Number); if(!y||!m||!d) return null; return new Date(y,m-1,d);}
function getMonthRange(y,m){ const start=new Date(y,m,1,0,0,0,0); const end=new Date(y,m+1,0,23,59,59,999); return {start,end};}
function getCurrentMonthValue(){ const now=new Date(); return `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}`;}

// ===== Dashboard protegido =====
onAuthStateChanged(auth, async user => {
  if(!user){ alert("Usuário não logado. Acesse pelo portal."); return; }
  CURRENT_USER = user;
  ...
  el("userNome") && (el("userNome").textContent = CURRENT_USER_DATA.nome || CURRENT_USER_DATA.matricula);

  qsel(".admin-only").forEach(b=>b.hidden=!IS_ADMIN);
  qsel(".user-only").forEach(b=>b.hidden=IS_ADMIN);

  // binds
  el("btnLogout")?.addEventListener("click", async()=>{ await signOut(auth); alert("Logout efetuado."); });
  el("btnAlterarSenha")?.addEventListener("click", async()=>{
    const nova = prompt("Nova senha:"); if(!nova) return;
    try{ await updatePassword(auth.currentUser,nova); alert("Senha alterada."); }catch(e){alert(e.message);}
  });
  ...
  ["valorFolha","valorDinheiro"].forEach(id=>{const i=el(id);i&&i.addEventListener("input",()=>{const vf=parseFloat(el("valorFolha").value||0); const vd=parseFloat(el("valorDinheiro").value||0); el("sobraFalta").value=BRL.format(vd-vf);})});
...
async function popularMatriculasSelects(){
  ...
  const selForm=el("matriculaForm"), selFiltro=el("filtroMatricula"), selResumo=el("selectMatriculas");
...
async function filtrarPorData(){
  const val=el("filtroDataGlobal").value; if(!val){alert("Escolha uma data."); return;}
...
async function carregarResumoAdmin(){
  ...
  const filtrados=rows.filter(r=>{ const sf=(r.valorDinheiro||0)-(r.valorFolha||0); if(tipo==="positivos") return sf>0; if(tipo==="negativos") return sf<0; return true;});
...
function renderLista(rows){
  ...
  rows.forEach(r=>{
    ...
    wrap.innerHTML=`<div class="item-header">
